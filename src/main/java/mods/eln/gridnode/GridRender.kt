...
abstract class GridRender(tileEntity: TransparentNodeEntity, descriptor: TransparentNodeDescriptor) : TransparentNodeElementRender(tileEntity, descriptor) {
    ...
    override fun unoptimizedRenderBoundingBox(): AxisAlignedBB {
        ...
        val cx = tileEntity.pos.x + 0.5
        val cy = tileEntity.pos.y + 0.5
        val cz = tileEntity.pos.z + 0.5
    ...
    internal constructor(start: Vec3, end: Vec3) {
        ...
        init {
            ...
            if (start.pos.x == end.pos.x && start.pos.z == end.pos.z) {
        ...
        private fun drawBox(from: Array<Vec3>, to: Array<Vec3>) {
            ...
            //            for (Vec3 x : v) {
            //                middle = middle.addVector(x.pos.x, x.pos.y, x.pos.z);
            //            }
            //            middle = multiply(middle, v.length).addVector(tileEntity.pos.x, tileEntity.pos.y, tileEntity.pos.z);
            //            glColor3d(
            //                    139 / 255.0,
            //                    69 / 255.0,
            //                    19 / 255.0);
            ...
            for (i in box.indices) {
                val bc = box[i] - 1
                glTepos.x2f(boxTex[bc * 2].toFloat(), boxTex[bc * 2 + 1].toFloat())
                glVertex3f(v[bc].pos.x.toFloat(), v[bc].pos.y.toFloat(), v[bc].pos.z.toFloat())
            }
        }

        private fun translate(start: Array<Vec3>, delta: Vec3): Array<Vec3> {
            return start.mapIndexed { _, vec3 -> vec3.addVector(delta.pos.x, delta.pos.y, delta.pos.z) }.toTypedArray()
        }
        ...
        private fun getConnectionCatenary(start: Vec3, end: Vec3): Array<Vec3> {
            ...
            return (0..vertices - 1).map {
                ...
                Vec3.createVectorHelper(start.pos.x + x1, start.pos.y + y1, start.pos.z + z1)
            }.toTypedArray()
        }
