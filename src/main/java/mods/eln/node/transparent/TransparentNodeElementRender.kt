...
abstract class TransparentNodeElementRender(var tileEntity: TransparentNodeEntity, var transparentNodeDescriptor: TransparentNodeDescriptor) {
    ...
    open fun unoptimizedRenderBoundingBox(): AxisAlignedBB {
        val base = AxisAlignedBB.getBoundingBox(
            (tileEntity.pos.x - 1).toDouble(),
            (tileEntity.pos.y - 1).toDouble(),
            (tileEntity.pos.z - 1).toDouble(),
            (tileEntity.pos.x + 1).toDouble(),
            (tileEntity.pos.y + 1).toDouble(),
            (tileEntity.pos.z + 1).toDouble()
        )
        ...
        val collisionQuery = AxisAlignedBB.getBoundingBox(
            (tileEntity.pos.x - 64).toDouble(),
            (tileEntity.pos.y - 64).toDouble(),
            (tileEntity.pos.z - 64).toDouble(),
            (tileEntity.pos.x + 64).toDouble(),
            (tileEntity.pos.y + 64).toDouble(),
            (tileEntity.pos.z + 64).toDouble()
        )
        ...
        if (ghostGroup != null) {
            for (element in ghostGroup.elementList) {
                val x = tileEntity.pos.x + element.x
                val y = tileEntity.pos.y + element.y
                val z = tileEntity.pos.z + element.z
    ...
    protected fun coordinate(): Coordinate {
        return Coordinate(tileEntity.pos.x, tileEntity.pos.y, tileEntity.pos.z, tileEntity.world())
    }
