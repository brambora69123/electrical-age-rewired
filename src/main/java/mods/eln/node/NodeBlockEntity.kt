    ...
    override fun serverPublishUnserialize(stream: DataInputStream) {
        ...
        try {
            ...
            if (redstone != newRedstone) {
                ...
                worldObj.notifyBlockChange(pos.x, pos.y, pos.z, getBlockType())
            } else {
                redstone = newRedstone
            }
        } catch (e: IOException) {
        ...
        {
			lastLight = 15;
			worldObj.updateLightByType(EnumSkyBlock.Block,pos.x,pos.y,pos.z);
		}*/if (lastLight != light) {
            lastLight = light
            worldObj.updateLightByType(EnumSkyBlock.Block, pos.x, pos.y, pos.z)
        }
    ...
    val node: Node?
        get() {
            ...
            if (internalNode == null) {
                val nodeFromCoordonate = NodeManager.instance!!.getNodeFromCoordonate(Coordinate(pos.x, pos.y, pos.z, worldObj))
                if (nodeFromCoordonate is Node) {
                    internalNode = nodeFromCoordonate
                } else {
                    println("ASSERT WRONG TYPE public Node getNode " + Coordinate(pos.x, pos.y, pos.z, worldObj))
                }
                if (internalNode == null) {
                    Utils.println("This is actually used?")
                    add(Coordinate(pos.x, pos.y, pos.z, worldObj))
                }
            }
            return internalNode
        }
