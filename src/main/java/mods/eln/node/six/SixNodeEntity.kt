...
class SixNodeEntity : NodeBlockEntity() {
    ...
    override fun serverPublishUnserialize(stream: DataInputStream) {
        ...
        //	worldObj.setLightValue(EnumSkyBlock.Sky, pos.x,pos.y,pos.z,15);
        if (sixNodeCacheBlock !== sixNodeCacheBlockOld) {
            val chunk = worldObj.getChunkFromBlockCoords(pos.x, pos.z)
            chunk.generateHeightMap()
            updateSkylight(chunk)
            chunk.generateSkylightMap()
            updateAllLightTypes(worldObj, pos.x, pos.y, pos.z)
        }
    }
