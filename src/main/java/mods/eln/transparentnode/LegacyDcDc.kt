...
class LegacyDcDcElement(transparentNode: TransparentNode, descriptor: TransparentNodeDescriptor): TransparentNodeElement(transparentNode, descriptor), IConfigurable {
    ...
    private fun computeInventory() {
        ...
        if (primaryCable == null || !hasValidCore || primaryCable.getCount() < 1) {
        ...
        if (secondaryCable == null || !hasValidCore || secondaryCable.getCount() < 1) {
        ...
        populated = primaryCable != null && secondaryCable != null && primaryCable.getCount() >= 1 && secondaryCable.getCount() >= 1 && hasValidCore

        ratioControl = if (populated) {
            secondaryCable!!.getCount().toDouble() / primaryCable!!.getCount().toDouble()
        } else {
            1.0
        }
    }
