        ...
        if (par1ItemStack.isStackable) {
            while (par1ItemStack.getCount() > 0 && (!par4 && k < par3 || par4 && k >= par2)) {
                ...
                if (slot.isItemValid(par1ItemStack) && itemstack1 != null && itemstack1.item === par1ItemStack.item && (!par1ItemStack.hasSubtypes || par1ItemStack.itemDamage == itemstack1.itemDamage) && ItemStack.areItemStackTagsEqual(
                    ...
                    val l = itemstack1.getCount() + par1ItemStack.getCount()
                    val maxSize = min(slot.slotStackLimit.toDouble(), par1ItemStack.maxStackSize.toDouble())
                        .toInt()
                    if (l <= maxSize) {
                        par1ItemStack.getCount() = 0
                        itemstack1.getCount() = l
                        slot.onSlotChanged()
                        flag1 = true
                    } else if (itemstack1.getCount() < maxSize) {
                        par1ItemStack.getCount() -= maxSize - itemstack1.getCount()
                        itemstack1.getCount() = maxSize
                        slot.onSlotChanged()
                        flag1 = true
                    }
                }
            ...
            while (!par4 && k < par3 || par4 && k >= par2) {
                ...
                if (itemstack1 == null && slot.isItemValid(par1ItemStack)) {
                    ...
                    if (l <= maxSize) {
                        ...
                        par1ItemStack.getCount() -= maxSize
                        val newItemStack = par1ItemStack.copy()
                        newItemStack.getCount() = maxSize
