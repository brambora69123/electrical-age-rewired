...
object UtilsClient {
    ...
    fun distanceFromClientPlayer(@Suppress("UNUSED_PARAMETER") world: World?, pos.x: Int, pos.y: Int, pos.z: Int): Float {
        val player = Minecraft.getMinecraft().thePlayer
        return Math.sqrt((pos.x - player.posX) * (pos.x - player.posX) + (pos.y - player.posY) * (pos.y - player.posY) + (pos.z - player.posZ) * (pos.z - player.posZ)).toFloat()
    }

    @JvmStatic
    fun distanceFromClientPlayer(tileEntity: SixNodeEntity): Float {
        return distanceFromClientPlayer(tileEntity.world(), tileEntity.pos.x, tileEntity.pos.y, tileEntity.pos.z)
    }
    ...
    fun drawHaloNoLightSetup(halo: Obj3DPart?, r: Float, g: Float, b: Float, e: TileEntity, bilinear: Boolean) {
        drawHaloNoLightSetup(halo, r, g, b, e.world(), e.pos.x, e.pos.y, e.pos.z, bilinear)
    }

    @JvmStatic
    fun drawHalo(halo: Obj3DPart?, r: Float, g: Float, b: Float, e: TileEntity, bilinear: Boolean) {
        drawHalo(halo, r, g, b, e.world(), e.pos.x, e.pos.y, e.pos.z, bilinear)
    }
    ...
    fun drawHaloNoLightSetup(halo: Obj3DPart?, r: Float, g: Float, b: Float, e: Entity, bilinear: Boolean) {
        ...
        // light =
        // e.world().getLightBrightnessForSkyBlocks(MathHelper.floor_double(e.posX),
    ...
    fun drawIcon(type: ItemRenderType) {
        ...
        when (type) {
            ItemRenderType.INVENTORY -> {
                ...
                GL11.glTepos.x2f(1f, 0f)
                GL11.glVertex3f(16f, 0f, 0f)
                GL11.glTepos.x2f(0f, 0f)
                GL11.glVertex3f(0f, 0f, 0f)
                GL11.glTepos.x2f(0f, 1f)
                GL11.glVertex3f(0f, 16f, 0f)
                GL11.glTepos.x2f(1f, 1f)
    ...
    fun drawItemStack(par1ItemStack: ItemStack?, x: Int, y: Int, @Suppress("UNUSED_PARAMETER") par4Str: String?, gui: Boolean) {
        ...
        if (par1ItemStack!!.getCount() > 1) {
            ...
            Minecraft.getMinecraft().fontRenderer.drawStringWithShadow("" + par1ItemStack.getCount(), x + 10, y + 9, -0x1)
            // GL11.glPopMatrix();
            enableDepthTest()
        }
    }
    ...
    fun clientDistanceTo(t: TransparentNodeEntity?): Double {
        ...
        val c: Entity = Minecraft.getMinecraft().thePlayer
        val x = c.posX - t.pos.x
        val y = c.posY - t.pos.y
        val z = c.posZ - t.pos.z
        return sqrt(x * x + y * y + z * z)
    }
