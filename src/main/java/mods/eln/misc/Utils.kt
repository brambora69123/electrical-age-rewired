...
object Utils {
    ...
    fun tryPutStackInInventory(stack: ItemStack, inventory: IInventory?): Boolean {
        ...
        var need = stack.getCount()
        run {
            var i = 0
            while (i < inventory.sizeInventory && need > 0) {
                val slot = inventory.getStackInSlot(i)
                if (slot != null && slot.getCount() < limit && slot.isItemEqual(stack)) {
                    slots.add(i)
        ...
        for (slot in slots) {
            ...
            if (target == null) {
                ...
                val space = limit - target.getCount()
        ...
        for (stack in inputStack) {
            ...
            for (idx in slotsIdList.indices) {
                ...
                if (targetStack == null) {
                    ...
                    val transferMax = limit - targetStack.getCount()
                    if (transferMax > 0) {
                        var transfer = stack!!.getCount()
                        if (transfer > transferMax) transfer = transferMax
                        outputStack[idx]!!.getCount() += transfer
                        stack.getCount() -= transfer
                    }
                    if (stack!!.getCount() == 0) {
                        oneStackDone = true
                        break
    ...
    fun unserializeItemStackToEntityItem(stream: DataInputStream, old: EntityItem?, tileEntity: TileEntity): EntityItem? {
        ...
        return if (stream.readShort().also { itemId = it }.toInt() == -1) {
            ...
            if (old == null || Item.getIdFromItem(old.entityItem.item) != itemId.toInt() || old.entityItem.itemDamage != ItemDamage.toInt()) EntityItem(tileEntity.world(), tileEntity.pos.x + 0.5, tileEntity.pos.y + 0.5, tileEntity.pos.z + 1.2, newItemStack(itemId.toInt(), 1, ItemDamage.toInt())) else old
        }
    }
    ...
    /*
	 * public static void drawIcon(Icon icon) { Utils.bindTextureByName(icon.getIconName()); Utils.disableCulling(); GL11.glBegin(GL11.GL_QUADS); GL11.glTepos.x2f(0f, 0f); GL11.glVertex3f(0.5f,-0.5f,0f); GL11.glTepos.x2f(0f, 0f);GL11.glVertex3f(-0.5f,-0.5f,0f); GL11.glTepos.x2f(0f, 1f);GL11.glVertex3f(-0.5f,0.5f,0f); GL11.glTepos.x2f(1f, 1f);GL11.glVertex3f(0.5f,0.5f,0f); GL11.glEnd(); Utils.enableCulling(); }
	 *
    ...
    @JvmStatic
    fun updateAllLightTypes(worldObj: World, pos.x: Int, pos.y: Int, pos.z: Int) {
        worldObj.func_147451_t(pos.x, pos.y, pos.z)
        worldObj.markBlocksDirtyVertical(pos.x, pos.z, 0, 255)
    }
