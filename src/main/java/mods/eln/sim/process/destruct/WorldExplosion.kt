...
class WorldExplosion : IDestructible {
    ...
    override fun destructImpl() {
        if (Eln.instance.explosionEnable) coordinate.world().createExplosion(
            null as Entity?,
            coordinate.pos.x.toDouble(),
            coordinate.pos.y.toDouble(),
            coordinate.pos.z.toDouble(),
            strength,
            true
        ) else coordinate.world().setBlockToAir(coordinate.pos)
    }
